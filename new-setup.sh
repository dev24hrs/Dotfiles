...
HOMEBREW_PREFIX="/opt/homebrew" # Apple Silicon; Intel 用 /usr/local

# ── symlink 映射表（与 cfg.sh 保持一致）─────────────────────────────────────
# 格式: "dotfiles 相对路径:目标绝对路径"
CONFIG_MAP=(
...
# 提示：.gitconfig 里硬编码了代理 127.0.0.1:12334，按需取消
if grep -q "proxy = 127.0.0.1" "$DOTFILES_DIR/git/.gitconfig" 2>/dev/null; then
  warn ".gitconfig 包含代理设置 (127.0.0.1:12334)，若当前环境无代理请手动注释:"
...
# tmux HEAD 需从 GitHub 源码编译，无 bottle，网络差时耗时较长
if grep -q 'brew "tmux".*HEAD' "$DOTFILES_DIR/Brewfile"; then
  info "注意: tmux 指定了 HEAD 版本，需要从源码编译，耗时较长..."
fi
...
if has rustc; then
  ...
  if [[ "$install_rust" =~ ^[Yy]$ ]]; then
    ...
    # 让当前 shell 能用 cargo
    source "$HOME/.cargo/env.fish" 2>/dev/null || true
...
info "安装 tmux 插件（tmux-sensible, vim-tmux-navigator）..."
